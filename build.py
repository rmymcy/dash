...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  body { display: grid; grid-template-columns: 320px 1fr 340px; min-height: 100vh; }
  @media (max-width: 1100px) { body { grid-template-columns: 280px 1fr; } #insight { display: none; } }
  @media (max-width: 760px)  { body { grid-template-columns: 1fr; } #filters { position: static; max-height: none; } }
...
<header>
  ...
  <span class="meta" title="Open Food Facts + OpenLabel grocery items, common US-retail brands (Walmart, Kroger, Target). All values per 100 g.">
    <span id="totalCount">—</span> items · per 100 g · open data
  </span>
  <div class="search">
    <input id="search" type="search" placeholder="Search by name, brand, category…" autofocus>
  </div>
</header>
...
function renderRows(vis) {
  ...
  for (const it of slice) {
    ...
    for (const k of NUT_KEYS) {
      ...
      let cls = "num";
      if (v === 0) cls += " zero";
      else if (v != null && n.dv > 0 && v > n.dv) cls += " over";
      tr.append(el("td", {class:cls, html: v == null ? "—" : (v >= 100 ? v.toFixed(0) : v >= 10 ? v.toFixed(1) : v.toFixed(2))}));
    }
    body.append(tr);
  }
